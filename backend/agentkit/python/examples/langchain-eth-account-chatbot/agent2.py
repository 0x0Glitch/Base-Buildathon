    ...
    return (
        create_react_agent(
            ...
            state_modifier=(
                ...
                "You also have special tools for interacting with the SuperETH contract on Base Sepolia - "
                "including crosschain minting and burning capabilities. "
                "You are the aiAgent () who can call these functions. "
...
def setup():
    ...
    chain_id = os.getenv("CHAIN_ID_BASE_SEPOLIA", "84532")  # Default to Base Sepolia
    ...
    if not os.path.exists(".env"):
        with open(".env", "a") as f:
            f.write("# SuperETH contract address on Base Sepolia\n")
            f.write("BASE_SEPOLIA_CONTRACT_ADDRESS=\n")
            f.write("# Base Sepolia RPC URL (optional)\n")
            f.write("BASE_SEPOLIA_RPC_URL=https://sepolia.base.org\n")
            print("Created .env file. Please add your BASE_SEPOLIA_CONTRACT_ADDRESS to it.")
    ...
    private_key = (
        os.getenv("BASE_SEPOLIA_PRIVATE_KEY")  # First priority: Environment variable
...
def main():
    ...
    # Check if BASE_SEPOLIA_CONTRACT_ADDRESS is set
    if not os.getenv("BASE_SEPOLIA_CONTRACT_ADDRESS"):
        print("Warning: BASE_SEPOLIA_CONTRACT_ADDRESS environment variable is not set.")
