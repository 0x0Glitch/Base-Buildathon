    ...
    return (
        create_react_agent(
            ...
            state_modifier=(
                ...
                "You also have special tools for interacting with the SuperETH contract on ARBITRUM Sepolia - "
...
def setup():
    ...
    chain_id = os.getenv("CHAIN_ID_ARBITRUM_SEPOLIA", "421614")  # Default to ARBITRUM Sepolia
    wallet_file = f"wallet_data_{chain_id}.txt"
    
    # Create a default .env file with ARBITRUM_CONTRACT_ADDRESS if it doesn't exist
    if not os.path.exists(".env"):
        with open(".env", "a") as f:
            f.write("# SuperETH contract address on ARBITRUM Sepolia\n")
            f.write("ARBITRUM_CONTRACT_ADDRESS=\n")
            f.write("# ARBITRUM Sepolia RPC URL (optional)\n")
            f.write("ARBITRUM_RPC_URL=https://sepolia.ARBITRUM.io\n")
            print("Created .env file. Please add your ARBITRUM_CONTRACT_ADDRESS to it.")
    ...
    private_key = (
        os.getenv("ARBITRUM_SEPOLIA_PRIVATE_KEY")  # First priority: Environment variable
...
def main():
    ...
    # Check if ARBITRUM_CONTRACT_ADDRESS is set
    if not os.getenv("ARBITRUM_CONTRACT_ADDRESS"):
        print("Warning: ARBITRUM_CONTRACT_ADDRESS environment variable is not set.")
