    ...
    return (
        create_react_agent(
            ...
            state_modifier=(
                ...
                "You also have special tools for interacting with the SuperETH contract on ETHEREUM Sepolia - "
...
def setup():
    ...
    chain_id = os.getenv("CHAIN_ID_ETHEREUM_SEPOLIA", "11155111")  # Default to ETHEREUM Sepolia
    wallet_file = f"wallet_data_{chain_id}.txt"
    
    # Create a default .env file with ETHEREUM_CONTRACT_ADDRESS if it doesn't exist
    if not os.path.exists(".env"):
        with open(".env", "a") as f:
            f.write("# SuperETH contract address on ETHEREUM Sepolia\n")
            f.write("ETHEREUM_CONTRACT_ADDRESS=\n")
            f.write("# ETHEREUM Sepolia RPC URL (optional)\n")
            f.write("ETHEREUM_RPC_URL=https://sepolia.ETHEREUM.io\n")
            print("Created .env file. Please add your ETHEREUM_CONTRACT_ADDRESS to it.")
    ...
    private_key = (
        os.getenv("ETHEREUM_SEPOLIA_PRIVATE_KEY")  # First priority: Environment variable
...
def main():
    ...
    # Check if ETHEREUM_CONTRACT_ADDRESS is set
    if not os.getenv("ETHEREUM_CONTRACT_ADDRESS"):
        print("Warning: ETHEREUM_CONTRACT_ADDRESS environment variable is not set.")
